...
url = str(os.getenv('HOST'))+target+'-subdomain-temp/_doc?refresh'
...
def sublist3r():
    subprocess.check_output('docker run --rm --name '+container_name+' -v '+homedir+'/recon/data/'+target+'/temp:/data kali-tool:2.0 sublist3r -d '+domain+' -o /data/'+output+' || true', shell=True)
